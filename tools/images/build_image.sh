...
# Go two levels up: tools/images -> tools -> fabric-x (repo root)
PROJECT_DIR=$(dirname "$(dirname "${SCRIPT_DIR}")")

# Debug log
echo "[build_image.sh] Using project root: ${PROJECT_DIR}"
...
# files
DOCKERFILE="${PROJECT_DIR}/tools/images/Dockerfile"
BUILD_ARGS=()
...
function parse_script_args() {
    while [[ "$#" -gt 0 ]]; do
       ...
       -f | --dockerfile)
            if [[ -n "$2" && "$2" != "--"* ]]; then
                DOCKERFILE="$2"
                # Keep PROJECT_DIR at repo root, do not reset
                shift
            else
                echo "Error: $2 option requires a value."
                exit 1
