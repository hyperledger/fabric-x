...
func (s *ethService) GetTransactionReceipt(r *http.Request, txID *string, reply *types.TxReceipt) error {
	...
	if to != "" {
		...
		if bytes.Equal(callee, ZeroAddress) {
			// Contract deployment: only report the deployed address when the tx
			// committed. A failed deployment (MVCC conflict, policy failure, etc.)
			// must not advertise a ContractAddress — no contract exists there.
			if txnValidValue == 1 {
	...
	// EIP-658: logs from a non-committed execution must not be surfaced.
	// Fabric embeds the endorser simulation events in every block transaction,
	// including ones later invalidated by MVCC or policy checks. Those state
	// changes were never applied to the ledger, so returning them here would
	// produce phantom events inconsistent with what eth_getLogs reports for the
	// same block range.
	if txnValidValue == 1 {
