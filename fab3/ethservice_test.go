...
var _ = Describe("Ethservice", func() {
	...
	Describe("GetTransactionReceipt", func() {
		...
		Context("when the transaction was invalidated by MVCC conflict", func() {
			var (
				eventBytes []byte
			)
			...
			BeforeEach(func() {
				...
				// Build a transaction with EVM events in the endorser response.
				chaincodeEvent := peer.ChaincodeEvent{
					ChaincodeId: "evmcc",
					TxId:        sampleTransactionID,
					EventName:   "Chaincode event",
					Payload:     []byte(`[{"address":"aabbcc","topics":["topic1"],"data":"deadbeef"}]`),
				}
				eventBytes, err = proto.Marshal(&chaincodeEvent)
				Expect(err).ToNot(HaveOccurred())
				...
				*sampleTransaction = *tx
				...
				block := GetSampleBlockWithTransaction(31, []byte("12345abcd"), sampleTransaction, otherTransaction)
				// Mark the first transaction (index 0) as MVCC-invalidated.
				block.Metadata.Metadata[common.BlockMetadataIndex_TRANSACTIONS_FILTER][0] = byte(peer.TxValidationCode_MVCC_READ_CONFLICT)
				mockLedgerClient.QueryBlockByTxIDReturns(block, nil)
			})

			It("returns status 0x0 with empty logs (no phantom events from uncommitted execution)", func() {
				var reply types.TxReceipt

				err := ethservice.GetTransactionReceipt(&http.Request{}, &sampleTransactionID, &reply)
				Expect(err).ToNot(HaveOccurred())

				Expect(reply.Status).To(Equal("0x0"), "MVCC-invalidated tx must have status 0x0")
				Expect(reply.Logs).To(BeNil(), "MVCC-invalidated tx must have no logs (EIP-658)")
				Expect(reply.ContractAddress).To(BeEmpty(), "MVCC-invalidated tx must not advertise a ContractAddress")
			})
		})

		Context("when a contract deployment was invalidated by MVCC conflict", func() {
			BeforeEach(func() {
				...
				*sampleTransaction = *tx
				...
				block := GetSampleBlockWithTransaction(31, []byte("12345abcd"), sampleTransaction, otherTransaction)
				// Mark the deployment tx (index 0) as MVCC-invalidated.
				block.Metadata.Metadata[common.BlockMetadataIndex_TRANSACTIONS_FILTER][0] = byte(peer.TxValidationCode_MVCC_READ_CONFLICT)
				mockLedgerClient.QueryBlockByTxIDReturns(block, nil)
			})

			It("returns status 0x0 and does not report a ContractAddress (contract was never deployed)", func() {
				var reply types.TxReceipt

				err := ethservice.GetTransactionReceipt(&http.Request{}, &sampleTransactionID, &reply)
				Expect(err).ToNot(HaveOccurred())

				Expect(reply.Status).To(Equal("0x0"))
				Expect(reply.ContractAddress).To(BeEmpty(), "failed deployment must not advertise a ContractAddress")
				Expect(reply.Logs).To(BeNil())
			})
		})
